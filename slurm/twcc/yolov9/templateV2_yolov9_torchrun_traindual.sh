...
DEVICE_LIST=$(seq -s, 0 $((GPUS_NUM-1)) | paste -sd, -)
...
TRAIN_CMD="torchrun --nproc_per_node=$GPUS_NUM --nnodes=$NNODES \
	...
	train_dual.py --workers 16 --device $DEVICE_LIST --sync-bn --batch $BATCH_NUM --data data/coco.yaml --img 640 \
	--cfg models/detect/yolov9-c.yaml --weights '' --name yolov9-c \
	--hyp hyp.scratch-high.yaml --min-items 0 --epochs 100 --close-mosaic 15"
